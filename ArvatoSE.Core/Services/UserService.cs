using DbController;
using System.Text;
using ArvatoSE.Core.Filters;
using ArvatoSE.Core.Models;

namespace ArvatoSE.Core.Services
{
    public class UserService : IModelService<User, int, UserFilter>
    {
        ...
        public async Task CreateAsync(User input, IDbController dbController, CancellationToken cancellationToken = default)
        {
            ...
            string sql = $@"INSERT INTO `arvato`.`users`
                (
                `username`,
                `display_name`,
                `description`,
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string sql = "DELETE FROM `arvato`.`users``WHERE `user_id` = @USER_ID";
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string sql = "DELETE FROM `arvato`.`user_friends` WHERE `user_id` = @USER_ID AND `friend_id` = @FRIEND_ID";
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string sql = @"SELECT * FROM `arvato`.`users` WHERE `user_id` = @USER_ID";
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string sql = @"SELECT * FROM `arvato`.`users` WHERE UPPER(username) = UPPER(@USERNAME)";
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string sql = @"SELECT `user_id`, `username`, `display_name`, `image` FROM `arvato`.`users` WHERE `user_id` = @USER_ID";
        ...
        public async Task<List<User>> GetAsync(UserFilter filter, IDbController dbController, CancellationToken cancellationToken = default)
        {
            ...
            sqlBuilder.Append("SELECT `user_id`, `username`, `display_name`, `description`, `main_fraction_id`, `last_login`, `image` FROM `arvato`.`users` WHERE 1 = 1");
            ...
            sql = "SELECT * FROM `arvato`.`user_permissions`";
...
(
    ...
    OR  UPPER(username) LIKE @SEARCHPHRASE
        ...
        public async Task<int> GetTotalAsync(UserFilter filter, IDbController dbController, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            StringBuilder sqlBuilder = new();
            sqlBuilder.AppendLine("SELECT COUNT(*) FROM `arvato`.`users` WHERE 1 = 1");
            sqlBuilder.AppendLine(GetFilterWhere(filter));
...
FROM arvato.users AS u
JOIN arvato.user_friends AS uf ON u.user_id = uf.friend_id
WHERE uf.user_id = @USER_ID";
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string sql = @"UPDATE `arvato`.`users` SET
`username` = @USERNAME,
...
WHERE user_id = @USER_ID";
        ...
        {
            string sql = "SELECT * FROM `arvato`.`users`";
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string sql = @"INSERT INTO `arvato`.`user_friends`
    (
    user_id,
    friend_id
    )
