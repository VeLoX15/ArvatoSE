...
namespace ArvatoSE.Core.Services
{
    public class PermissionService
    {
        public async Task<List<Permission>> GetUserPermissionsAsync(int userId, IDbController dbController, CancellationToken cancellationToken = default)
        {
            ...
            string sql = @"SELECT p.*
                FROM `arvato`.`user_permissions` up
                INNER JOIN `arvato`.`permissions` p ON (p.`permission_id` = up.`permission_id`)
        ...
        public async Task UpdateUserPermissionsAsync(User user, IDbController dbController, CancellationToken cancellationToken = default)
        {
            ...
            string sql = "DELETE FROM `arvato`.`user_permissions` WHERE `user_id` = @USER_ID";
            ...
            {
                sql = @"INSERT INTO `arvato`.`user_permissions`
    (
    user_id,
    permission_id
    )
    ...
    @PERMISSION_ID
        ...
        {
            string sql = "SELECT * FROM `arvato`.`permissions`";
        ...
        private static async Task LoadPermissionDescriptionsAsync(List<Permission> list, IDbController dbController, CancellationToken cancellationToken = default)
        {
            ...
            if (list.Any())
            {
                ...
                string sql = $"SELECT * FROM `arvato`.`permission_description` WHERE `permission_id` IN ({string.Join(",", permissionIds)})";
